extends Node2D
#TODO: Turns are being skipped, energy not displayed correctly, # Implementing status attacks
...
func _setup_minions(team : Array[MinionData],teamIndex : int):
	...
	for minion in team:
		...
		minion_instance.minion_stat_changed.connect(_minion_stat_changed)
		
		cycle_complete.connect(minion_instance.update_overtime_effects)
		move_complete.connect(minion_instance.move_compete)
...
func _minion_stat_changed(stat : String):
	if stat == "speed":
		var cur_min = turn_order[turn]
		_set_turn_order()
		turn = turn_order.find(cur_min)
